...
def simple(coordinaten,stepmatrix,calculatiematrix,end):
    ...
    calculatiematrix = matrix_replacevalue(calculatiematrix,h,coordinaten[0],coordinaten[1])

    return calculatiematrix,h
...
def calculate(omgevingsmatrix,calcultionmatrix,stepmatrix,start,stop):
    ...
    for coordinaat in filter_coord_list:
        ...
        calcultionmatrix, value = simple(coordinaat,stepmatrix,calcultionmatrix,stop)
...
def simulate(omgevingsmatrix,start,stop,updatesize=1):
    ...
    DISPLAY=pygame.display.set_mode((len(omgevingsmatrix[0])*ver_factor,len(omgevingsmatrix)*ver_factor),FULLSCREEN | DOUBLEBUF,32)
